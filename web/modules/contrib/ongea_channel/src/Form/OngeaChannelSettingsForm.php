...
namespace Drupal\ongea_channel\Form;
...
/**
 * Class OngeaChannelSettingsForm.
 *
 * @package Drupal\ongea_channel\Form
 */
class OngeaChannelSettingsForm extends ConfigFormBase
{
    ...
    {
        return [
            'ongea_channel.ongeaconfigsettings',
        ];
    }
    ...
    {
        return 'ongea_channel_config_form';
    }
    ...
    {
        $config = $this->config('ongea_channel.ongeaconfigsettings');
        ...
        $form['languages'] = $this->buildLanguageFormPart($domain_id, $config);
        ...
        $form['colors']['front'] = array(
            '#type' => 'textfield',
            '#title' => $this->t('Default Front Color'),
            '#default_value' => ($config->get($domain_id) !== NULL) ? $config->get($domain_id . '.color_front') : $site_config->get('color.front'),
            '#size' => 40,
            '#description' => $this->t(''),
        );


        /*$form['languages']['select'] = array(
            '#type' => 'checkboxes',
     ...
     * build language key
     */
    protected function buidLanguageKey($domain_id, $langKey) {
        return $domain_id . '.language.' . $langKey;
    }

    /**
     * build languages form part
     */
    public function buildLanguageFormPart($domain_id, $config) {
        ...
        foreach ($languages as $langKey => $language) {
            $formLanguages['lang_' . $langKey] = array(
                '#type' => 'checkbox',
                '#title' => $this->t($langKey),
                '#default_value' => $config->get($this->buidLanguageKey($domain_id, $langKey)) === 1 ? TRUE : FALSE,
    ...
    /**
     * build domain language config object
     */
    protected function buildDomainLanguageConfigObjct($domain_id, FormStateInterface $form_state) {
        $domain_part = $domain_id;

        $general_part = '.language.entity.';
    ...
    public function submitForm(array &$form, FormStateInterface $form_state)
    {
        ...
        $configLanguages = \Drupal::languageManager()->getLanguages();
        $formLanguages = [];
        foreach ($configLanguages as $langKey => $language) {
            $langVal = $form_state->getValue('lang_' . $langKey);
            $formLanguages[$langKey] = $langVal;
        }
        ...
        foreach ($formLanguages as $langKey => $language) {
            $config->set($domain_id . '.language.' . $langKey , $language);
        }
        ...
        //$config->set($domain_id . '.languages.de', $form_state)
        $config->save();
